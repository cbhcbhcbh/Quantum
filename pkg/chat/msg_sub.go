...
func NewMessageSubscriber(router *message.Router, config *config.Config, sub message.Subscriber, m MelodyChatConn) (*MessageSubscriber, error) {
...
func (s *MessageSubscriber) sendMessage(msg *domain.Message) error {
	return s.m.BroadcastFilter(msg.ToPresenter().Encode(), func(sess *melody.Session) bool {
		channelID, exist := sess.Get(sessCidKey)
		if !exist {
			return false
		}
		return msg.ChannelID == channelID.(uint64)
	})
}
